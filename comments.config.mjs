...
export const resolvedConfigData = {
  "jsDoc": {
    "definitions": {
      ...
      "getEnvironmentFromResolvedDirective": {
        "value": "Gets the environment from a resolved directive (like `\"use server logics\"` -> `\"server\"`). This can either be one of the Concrete Environments (the Server Environment or the Client Environment), or the Abstract Environment (the Agnostic Environment).",
        "key": "JSDOC#DEFINITIONS#GETENVIRONMENTFROMRESOLVEDDIRECTIVE"
      },
      "defineDirective21": {
        "value": "Creates the ESLint config object required as the basis for the Directive-First Architecture, linting server-client-agnostic imports based on their commented directives. (Defaults to `\"warn\"`. You can import and use ```[`${useAgnosticPluginName}/${directive21ConfigName}`]``` later in a further ESLint config object to modify that value.)",
        "key": "JSDOC#DEFINITIONS#DEFINEDIRECTIVE21"
      },
      "defineConfigSettings": {
        "value": "Defines the config settings for the eXtra JSX VS Code extension as a means to configure `eXtra JSX` directly from ESLint, given the fact that `eslint-plugin-use-agnostic` and `eXtra JSX` have to work together in making the Directive-First Architecture.",
        "key": "JSDOC#DEFINITIONS#DEFINECONFIGSETTINGS"
      },
      "agnostic20": {
      ...
      "directive21": {
        ...
        "nameIsPascalCase": {
          "value": "Checks if the name of an identifier is in PascalCase, as a cheap way to assess whether or not it is a React component.",
          "key": "JSDOC#DEFINITIONS#DIRECTIVE21#NAMEISPASCALCASE"
        },
        "declaresChildrenProp": {
          "value": "Checks whether the parameters of a function (React component) include the `children` property.",
          "key": "JSDOC#DEFINITIONS#DIRECTIVE21#DECLARESCHILDRENPROP"
        },
        "functionDeclarationFlow": {
          "value": "The flow for function declarations to ensure that Client Lineals Components are made in Client Components Modules and Client Contexts Components are made in Client Contexts Modules, based on the fact that the former are child-free and the latter are children-bearing.",
          "key": "JSDOC#DEFINITIONS#DIRECTIVE21#FUNCTIONDECLARATIONFLOW"
        },
        "jsxElementFlow": {
          "value": "The flow for JSX elements to ensure that render props are only used isolated inside a Client Components Module, where their execution would safely be happening exclusively on the client.",
          "key": "JSDOC#DEFINITIONS#DIRECTIVE21#JSXELEMENTFLOW"
        },
        "jsxOpeningElementFlow": {
          "value": "The flow for JSX opening elements to ensure that event handler props on intrinsic elements are only used in modules that can safely create Client Components.",
          "key": "JSDOC#DEFINITIONS#DIRECTIVE21#JSXOPENINGELEMENTFLOW"
        }
      },
      "tests": {
    ...
    "params": {
      ...
      "resolvedDirective": {
        "value": "The resolved directive at hand.",
        "key": "JSDOC#PARAMS#RESOLVEDDIRECTIVE"
      },
      "settings": {
        "value": "The settings as follows:",
      ...
      "directive21": {
        ...
        "name": {
          "value": "The name of the identifier at hand.",
          "key": "JSDOC#PARAMS#DIRECTIVE21#NAME"
        },
        "params": {
    ...
    "returns": {
      ...
      "getEnvironmentFromResolvedDirective": {
        "value": "Either `\"server\"`, `\"client\"`, or `\"agnostic\"`.",
        "key": "JSDOC#RETURNS#GETENVIRONMENTFROMRESOLVEDDIRECTIVE"
      },
      "defineDirective21": {
        "value": "An ESLint config object that applies `eslint-plugin-use-agnostic`'s `directive21` config by using the provided `reactFolder` as the basis for JavaScript/TypeScript glob patterns.",
        "key": "JSDOC#RETURNS#DEFINEDIRECTIVE21"
      },
      "defineConfigSettings": {
        "value": "The config object responsible for the settings retrieved by the eXtra JSX VS Code extension.",
        "key": "JSDOC#RETURNS#DEFINECONFIGSETTINGS"
      },
      "agnostic20": {
        "makeAgnostic20Config": {
      ...
      "directive21": {
        ...
        "nameIsPascalCase": {
          "value": "`true` if the name is recognized as PascalCase, `false` otherwise.",
          "key": "JSDOC#RETURNS#DIRECTIVE21#NAMEISPASCALCASE"
        },
        "declaresChildrenProp": {
          "value": "`true` if the parameters include the `children` property, `false` otherwise.",
          "key": "JSDOC#RETURNS#DIRECTIVE21#DECLARESCHILDRENPROP"
        }
      },
      "tests": {
        "readFilesRecursively": {
