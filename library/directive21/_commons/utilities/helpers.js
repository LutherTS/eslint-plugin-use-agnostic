...
export const getCommentedDirectiveFromSourceCode = (sourceCode) => {
  ...
  // Obtains the value depending on whether the raw value is single- or double-quoted. (Note: $COMMENT#JSDOC#FORCOMPOSEDVARIABLES#INITIALTHE same string is returned if, for some impossible reason, the raw value does not correspond in terms of quote types. It does not matter because the check coming next will always fail to null if that's the case.)
  const value = isSingleQuoted
    ? stripSingleQuotes(rawValue)
    : stripDoubleQuotes(rawValue);

  // certifies the $COMMENT#JSDOC#FORCOMPOSEDVARIABLES#DIRECTIVE or lack thereof from the obtained value
 ...
 * @param {CommentedDirective} $COMMENT#JSDOC#FORCOMPOSEDVARIABLES#DIRECTIVE $COMMENT#JSDOC#PARAMS#DIRECTIVE21#DIRECTIVE
...
export const getStrategizedDirective = (context, node) => {
  ...
  // maps the strategy to the its relevant $COMMENT#JSDOC#FORCOMPOSEDVARIABLES#DIRECTIVE
